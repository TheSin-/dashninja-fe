...
$(document).ready(function(){
    ...
    $('#proposalsdetailtable').on('xhr.dt', function ( e, settings, json ) {
        ...
        var nextsuperblockdatetimestamp = json.data.stats.latestblock.BlockTime+(((json.data.stats.nextsuperblock.blockheight-json.data.stats.latestblock.BlockId)/685.71)*86400);
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        if (type == "sort") {
                            ...
                            return '<a href="' + trcninjagovernanceproposaldetail[trcninjatestnet].replace('%%b%%', encodeURIComponent(data.PaymentProposalHash)) + '">' + data.PaymentProposalName + '</a>';
                        }
                    }
                    },
                    {
                        ...
                        if (type == "sort") {
                            ...
                            return addCommas(data.PaymentAmount.toFixed(3)) + " " + trcninjacoin[trcninjatestnet];
                        }
                    }
                    }
                ],
        ...
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
                ...
                if (type != 'sort') {
                    ...
                    outtxt = '<a href="'+trcninjagovernanceproposaldetail[trcninjatestnet].replace('%%b%%',data.Hash)+'">'+outtxt+'</a>';
//                    outtxt = '<a href="'+data.URL+'">'+outtxt+'</a>';
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.PaymentAmount.toFixed(2))+'&nbsp;'+trcninjacoin[trcninjatestnet];
                }
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    if (data.SuperBlockVersion == 1) {
                        return '<a href="' + trcninjabudgetdetail[trcninjatestnet].replace('%%b%%', encodeURIComponent(data.SuperBlockProposalName)) + '">' + data.SuperBlockProposalName + '</a>';
                    }
                    else {
                        return '<a href="' + trcninjagovernanceproposaldetail[trcninjatestnet].replace('%%b%%', encodeURIComponent(data.SuperBlockProposalHash)) + '">' + data.SuperBlockProposalName + '</a>';
                    }
                }
              }
            },
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    for ( var i=0, ien=trcninjaaddressexplorer[trcninjatestnet].length ; i<ien ; i++ ) {
                        if (ix == 0) {
                            outtxt += '<a href="'+trcninjaaddressexplorer[trcninjatestnet][0][0].replace('%%a%%',data.SuperBlockPaymentAddress)+'">'+data.SuperBlockPaymentAddress+'</a>';
                        }
                        else {
                            outtxt += '<a href="'+trcninjaaddressexplorer[trcninjatestnet][i][0].replace('%%a%%',data.SuperBlockPaymentAddress)+'">['+ix+']</a>';
                        }
                        ix++;
                    }
                    return outtxt;
